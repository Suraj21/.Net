...
namespace DelegateAndEvents
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //VideoEncoded behind the scene is a list of pointer to method, when the videoEncoder wantes to publish the event it looks the list (i.e at if(VideoEncoded != null)) if it is not null then it means 
            //that we have a pointer to an event handler method 
            videoEncoder.VideoEncoded += mailService.OnVideoEncode;//Since we are not writing it like a method this means this is actully a reference to the method which delegate id defining
